...
| Timeout default | 20 minutes; `--timeout=<minutes>` flag > `config.loop.timeoutMinutes` > default. `0` disables. |
...
### 3. `src/loop/runner.ts` — agent-run timeout
`Invocation` gains an optional `timeoutMs?: number`. `runCli` passes it to
`execSync`/`execFileSync` as the `timeout` option (Node kills the child on expiry and throws).
`makeRunner`/`makeReviewRunner` accept a `timeoutMs` and thread it into the invocation; on a
timeout the thrown error is caught as today → `{ success: false, summary: 'timed out after Nm' }`
→ the loop blocks. The reviewer run uses the same timeout.

### 4. `src/loop/run-command.ts` — wire + upgrade `loop status`
- `runLoopCommand` resolves the timeout (`--timeout` flag > `config.loop.timeoutMinutes` >
  default 20; `0` disables), builds the real `makeReporter(targetDir)`, builds runners with
  `timeoutMs`, and passes the reporter into `runLoop`.
...
### 5. `src/retrofit/config.ts` — optional config field
`loop.timeoutMinutes?: number` added to the schema (optional, backwards-compatible).

### 6. `src/cli.ts` — flag + gitignore note
...
storyStart(S6) ─► [console ▶ / status running:implementing / log]
   runner(timeoutMs) ─► phase(verifying) ─► verify ─► phase(reviewing) ─► [review]
...
- **runner.ts:** `timeoutMs` threaded into the `Invocation`; resolution order flag>config>20;
  `0` → no timeout passed.
...
```
…instead of `18/45`. And a true hang would self-terminate after 20 minutes as `blocked`.
