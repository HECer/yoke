...
export function planQwen(canonDir: string, targetDir: string, codeGraph: CodeGraph = 'graphify', codeIntelligence: CodeIntelligenceMode = 'off'): Action[] {
  const manifest = loadManifest(join(canonDir, 'manifest.yaml'))
  const actions: Action[] = []

  // QWEN.md: baseline + instruction for commands outside the rewrite hook.
  ...
  for (const skill of manifest.skills) {
    ...
    actions.push({
      kind: 'write',
      target: `.qwen/commands/${skill.id}.toml`,
      content: `description = "${description.replace(/\\/g, '\\\\').replace(/"/g, '\\"')}"\nprompt = ${tomlString(prompt)}\n`,
      reason: `qwen command: ${skill.id}`,
    })
    actions.push(...skillPackageActions(canonDir, skill, 'qwen'))
  }
